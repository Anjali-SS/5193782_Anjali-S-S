...
DoublyLinkedListNode* reverse(DoublyLinkedListNode* llist) {
 if (llist == NULL) return NULL; // empty list case
    ...
    while (current != NULL) {
        ...
        current->next = temp;
        current = current->prev;  // move to "next" (which was prev before swap)
    }

    // After the loop, temp will be pointing to the new head's prev
    if (temp != NULL) {
        llist = temp->prev;
    }
