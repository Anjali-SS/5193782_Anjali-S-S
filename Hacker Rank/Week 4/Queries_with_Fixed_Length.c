...
int* solve(int arr_count, int* arr, int queries_count, int* queries, int* result_count) {
    ...
    *result_count = queries_count;
    ...
    for (int qi = 0; qi < queries_count; qi++) {
        ...
        for (int i = 0; i < k; i++) {
            while (dq->size > 0 && arr[i] >= arr[dq->data[dq->rear]]) popBack(dq);
            pushBack(dq, i);
        }

        // Process rest of the array
        for (int i = k; i < arr_count; i++) {
            // Maximum of previous window
            if (arr[dq->data[dq->front]] < minOfMax) minOfMax = arr[dq->data[dq->front]];

            // Remove elements out of current window
            while (dq->size > 0 && dq->data[dq->front] <= i - k) popFront(dq);
