...
char* highestValuePalindrome(char* s, int n, int k) {
    ...
    // Step 1: Make it a palindrome (minimum changes)
    while (left < right) {
        if (result[left] != result[right]) {
            char maxDigit = (result[left] > result[right]) ? result[left] : result[right];
            result[left] = result[right] = maxDigit;
    ...
    // Step 2: Maximize palindrome value using remaining changes
    left = 0;
    right = n - 1;
    int remaining = k - minChanges;

    while (left <= right) {
        if (left == right) { // Middle character for odd-length strings
